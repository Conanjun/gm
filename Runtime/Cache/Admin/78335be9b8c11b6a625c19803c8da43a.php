	...
	$('form').validate({
        ...
        rules: {
            title: {
                required: true,
                remote: {
                    url: "<?php echo U('AuthManager/check_title');?>",
                    type: 'post',
                    data: {
                        title: function () {
                            return $('#title').val();
            ...
            <div class="col-lg-12">
                <div class="ibox float-e-margins">
                    ...
                    <div class="ibox-content">
                        ...
                        <form action="<?php echo U('AuthManager/edit');?>" method='post' id="form" class="form-horizontal" enctype="multipart/form-data">
						<input type="hidden" name="id" value="<?php echo ($group["id"]); ?>">
                            <div class="form-group">
                                <label class="col-sm-2 control-label"><?php echo L('PLATFORM_NAME');?></label>
                                <div class="col-sm-3">
                                    <input type="text" name="title" id="title" value="<?php echo ($group["title"]); ?>" class="form-control" autocomplete="off" placeholder="<?php echo L('PLATFORM_NAME_NOT_EMPTY');?>">
                                </div>
                            </div>
							<div class="form-group">
                                <label class="col-sm-2 control-label"><?php echo L('DESCRIPTION');?></label>
                                <div class="col-sm-4">
                                    <textarea id="description" name="description" class="form-control"><?php echo ($group["description"]); ?></textarea>
                                </div>
                            </div>
							 <div class="form-group">
                                <label class="col-sm-2 control-label"><?php echo L('STATE');?></label>
                                <div class="col-sm-3">
                                    <div class="radio radio-inline"><input type="radio" value="1" name="status" <?php if($group["status"] == 1): ?>checked<?php endif; ?>> <label> <?php echo L('NORMAL');?> </label></div>
                                    <div class="radio radio-inline"><input type="radio" value="0" name="status" <?php if($group["status"] == 0): ?>checked<?php endif; ?>> <label> <?php echo L('DISABLE');?> </label></div>
                                </div>
                            </div>
                            <div class="form-group">
...
<div class="wrapper wrapper-content">
					...
					<table
						...
						<tbody>
							<?php if(is_array($node_list)): $i = 0; $__LIST__ = $node_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$node): $mod = ($i % 2 );++$i;?><tr class="main_row">
								...
								<td><?php if(isset($node['child'])): if(is_array($node['child'])): $i = 0; $__LIST__ = $node['child'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$child): $mod = ($i % 2 );++$i;?><div class="rule_check">
										...
										<?php if(!empty($child['operator'])): ?><span
											...
											<?php if(is_array($child['operator'])): $i = 0; $__LIST__ = $child['operator'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$op): $mod = ($i % 2 );++$i;?><label
												style="width: 120px; color: #666"<?php if(!empty($op['tip'])): ?>title='<?php echo ($op["tip"]); ?>'<?php endif; ?>> <input
												class="auth_rules" type="checkbox" name="rules[]" 
												value="<?php echo $auth_rules[$op['url']] ?>" disabled /> <?php echo L($op['title']);?>
											</label><?php endforeach; endif; else: echo "" ;endif; ?> </span><?php endif; ?>
    ...
    <!--<script src="/static/thinkbox/jquery.thinkbox.js"></script>-->

     <script type="text/javascript">
         $(function(){
             $("#list<?php echo ($id); ?>").attr('class','active');

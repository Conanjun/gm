...
class MenuController extends AdminController
{
    ...
    {
        $config = array(
            "actionURL" => "/Admin/Menu/index",
            "operators" => array("include" => "包含", "=" => "=", "!=" => "!=", ">" => ">", ">=" => ">=", "<" => "<", "<=" => "<="),
            "params" => array(
                0 => array('str' => '名称', 'fieldname' => 'title', 'operator' => 'include', 'control' => 'input', 'values' => '')
            , 1 => array('str' => 'URL', 'fieldname' => 'url', 'operator' => '=', 'control' => 'input', 'values' => "")
            , 2 => array('str' => '上级菜单', 'fieldname' => 'pid', 'operator' => '=', 'control' => 'input', 'values' => '')
            , 3 => array('str' => '开发可见', 'fieldname' => 'is_dev', 'operetor' => '', 'control' => 'select', 'values' => array("" => "", '0' => '否', '1' => '是'))
            ));
        ...
        $pid = I('get.pid', 0);
        if ($pid) {
            $data = M('Menu')->where("id={$pid} and " . $where['_string'])->field(true)->order($order)->find();
...
//        $all_menu   =   M('Menu')->getField('id,title');
        ...
        $list = M("Menu")->where("pid={$pid} and " . $where['_string'])->field(true)->order($order)->select();
...
//            var_dump($data);exit;
            ...
            $root_mbx[] = array('title' => '管理', 'url' => U('ProjectConfig/index'));
            $root_mbx[] = array('title' => '菜单', 'url' => U('Menu/index'));
            $root_mbx[] = array('title' => '编辑菜单', 'url' => '');
            $this->assign('root_mbx', $root_mbx);

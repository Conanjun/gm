...
class RegionController extends AdminController
{
    ...
    function edit()
    {
        if (!IS_POST)
        {
			...
			$pr=$this->model->where("region_id='{$id}'")->find();
			$lr="";
			if($pr["path"]){
				$array=explode(",", $pr["path"]);
				foreach ($array as $v){
					$row=$this->model->where("region_id='{$v}'")->find();
					$lr.=$row["region_name"]." ";
				}
			}
        ...
        else
        {
            ...
            $pr=$this->model->where("region_id='{$_POST['id']}'")->find();

            /* 检查名称是否已存在 */
            if ($this->model->unique(trim($data['region_name']), $pr["parent_id"]))
            {
				$this->error(L('NAME_EXISTED'));
                return;
            }
...
//             );
   ...
   function ajax_col()
   {
    ...
    public function checkname(){
    	$name=I("get.region_name","");
    	$id=I("get.regionid","");
    	
    	$id=explode(",", $id);
    	$id=end($id);
    	
    	if ($this->model->unique(trim($name), $id))
    	{
    ...
    public function deletedata()
    {
    	...
    	foreach($ids as $id){
    		...
    		if(empty($list)){
    			...
    			$res = $this->model->where("path like '%,{$id},%'")->delete();
    			$sucmsg.="{$list['region_name']}".L("DELETE_SUCCESS")."<br/>";
    			action_log_new(array('outtype' => 'ot_region', 'outkey' => $id, 'action' => 'delete', 'comment' => ''));
    		}
    	}

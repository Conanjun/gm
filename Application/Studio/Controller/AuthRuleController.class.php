...
class AuthRuleController extends AdminController
{
    ...
    public function index()
    {
        ...
        $groupM = M('auth_group');
		$where = " 1=1 and module = 'Studio' and stuid = {$stuid} and status =1";
    ...
    public function add()
    {
        ...
        if (IS_POST) {
            ...
            $res = $auth_group->where('stuid =' . $stuid)->getByTitle($data['title']);
            if ($res) {
    ...
    public function check_title()
    {
        ...
        $id = $_POST['id'];
        if ($id) {
            if (M('Auth_group')->where("title='{$title}' and stuid = '{$stuid}' and id !=" . $id )->find())
                echo 'false';
            else
                echo 'true';
        } else {
            if (M('Auth_group')->where(['title' => $title,'stuid'=>$stuid])->find())
                echo 'false';
            else {
                echo 'true';
            }
    ...
    public function edit()
    {
        ...
        $id = I('get.id');
        $list = M('AuthGroup')->where(array('status' => array('egt', '0'), 'id' => $id))->find();

        if($list['fromadmin'] == 2){
            $this->error(L('EDIT_NO_MSG'));
        }
        ...
        if (IS_POST) {
            ...
            $res = $group->where('stuid =' . $stuid)->where('id =' . $id)->getByTitle($data['title']);
            if ($res) {
                if($res['id'] != $id){
                    $this->error(L('PLATFORM_NAME_EXISTED'));
                }
            ...
            if ($data) {
                ...
                if ($group->where('id = ' . $id)->save($data) !== false) {
                    ...
                    $this->redirect('AuthRule/edit?id=' . $id);
                } else {
    ...
    public function deluser()
    {
    	...
    	if($id&&$uid){
    		M("AuthGroupAccess")->where("group_id='{$id}' and uid='{$uid}'")->delete();
    	}
    	 
    	 
    	$this->json_success("suc");
    }
    ...
    public function delete()
    {
    	...
    	foreach($ids as $id){
    		$where = "id = '{$id}'";
    		$list= $groupM->where($where)->find();
    		//人员
    		$sql="select 1 from ot_auth_group_access g inner join ot_ucenter_member um on g.uid=um.id
			inner join ot_member m on um.id = m.uid left join ot_dept d on d.did=m.dept
			where m.status=1 and g.group_id='" . $id . "' limit 1 ";
    		$users = M('auth_group_access')->query($sql);
    		if($users){
    			$failmsg.="{$list['title']}".L("DEL_FAIL_PLAT")."<br/>";
    		}else{
    			$res = $groupM->where($where)->delete();
    			action_log_new(array('outtype' => 'ot_auth_group', 'outkey' => $id, 'action' => 'delete', 'comment' => ''));
    			$sucmsg.="{$list['title']}".L("DELETE_SUCCESS")."<br/>";
    		}
    	}
    ...
    public function delete1()
    {
        ...
        foreach ($res_ids as $vo) {
            ...
            if ($res_check) {
                $this->error(L('DELETE_FAILED_MSG'));
            }
        }
    ...
    public function userlist(){
        ...
        if(IS_POST){
            ...
            $ids=I("post.pro","");
            if($ids){
                $acc=M("AuthGroupAccess");
                foreach ($ids as $v){
                    if($v){
                        $row=$acc->where("uid='{$v}' and group_id='{$id}'")->find();
                        if(empty($row)){
                            $a=array("uid"=>$v,"group_id"=>$id);
                            $acc->add($a);
                        }
                    }
                }
            ...
            $this->redirect('AuthRule/userlist?id=' . $id);
            ...
            $new_user = M('ucenter_member')->field('um.*,m.nickname,m.uid')->alias('um')
            ->join("ot_member m on m.uid=um.id and m.status=1 and m.module = 'Studio' and m.stuid = {$stuid}")
            ->where($map_usr)->select();
            
            $ulist=M("UcenterMember")->alias("um")->join("inner join ot_member m on m.uid=um.id")->field("m.uid,m.nickname")->where(['m.module'=>'Studio','stuid'=>$stuid])->select();
            $this->assign('ulist',$new_user);

            $lists=M("AuthGroupAccess")->field("m.uid,m.nickname")->alias("aa")->join("inner join ot_member m on m.uid=aa.uid")->where("aa.group_id='{$id}'")->select();
            $this->assign('lists',$lists);
            $this->display();
        }
    }
    ...
    {	
        ...
        if ($data) {
            ...
            if ($r === false) {
                ...
                $this->redirect('AuthRule/edit?id=' . $data['id']);
            }
        } else {
            $this->error('操作失败' . $AuthGroup->getError());
        }

...
class UserController extends AdminController
{
    ...
    public function index()
    {
        ...
        $where = " ot_member.module = 'Studio' and ot_member.stuid ={$stuid}";
        $filter= '';
        ...
        $count = M("Member")->join("left join ot_dept d on d.did=ot_member.dept")
            ...
            ->join('left join ot_auth_group ag on  ag.id=aga.group_id')->where($where)->count($where);
        ...
        $roles = M('Auth_group')->field(true)->where("module='Studio' and stuid = {$stuid} and status =1")->select();
	...
	public function updatepassword() {
		if (IS_POST) {
			try {
				$password = I ( 'post.old' );
				if(empty ( $password )){
					throw new Exception( L("PLEASEINPUTOLDPWD") );
				}
				$data ['password'] = I ( 'post.password' );
				if(empty ( $data ['password'] )){
					throw new Exception( L("PLEASEINPUTNEWPWD"));
				}
				$repassword = I ( 'post.repassword' );
				if(empty ( $repassword )){
					throw new Exception( L("REPWDRE"));
				}
				
				if ($data ['password'] !== $repassword) {
					throw new Exception( L("PWDNOTSAME"));
				...
				$this->json_error($e->getMessage(),false);
			}
		} else {
			$this->display ();
		}
    ...
    public function add($username = '', $password = '', $repassword = '', $email = '', $mobile = '')
    {
        if (IS_POST) {
            ...
            if (0 < $uid) { //注册成功
                $user = array('uid' => $uid, 'nickname' => $nickname, 'status' => 1, 'sex' => $sex, 'birthday' => $birthday, 'stuid' =>$stuid,'module'=>'Studio');

                if (!M('Member')->add($user)) {
                    $this->error(L('ADD_FAILED'));
                } else {
                    $this->redirect('Studio/User/index');
                }
            } else { //注册失败，显示错误信息
...
//		        $password=think_ucenter_md5($pwd);
    ...
    public function delete()
    {
		...
		foreach($ids as $id){
			$where = "uid = {$id}";
			$list =M('Member')->where($where)->find();
			if(empty($list)){
				$failmsg.= "ID".$id.L("DELETE_FAILED")."<br>";
			}else{
				M('Member')->where($where)->delete();
				M('UcenterMember')->where('id = ' . $id)->delete();
				M('auth_group_access')->where($where)->delete();
				$sucmsg.="{$list['nickname']}".L("DELETE_SUCCESS")."<br/>";
				action_log_new(array('outtype' => 'ot_member', 'outkey' => $id, 'action' => 'delete', 'comment' => ''));
			}
		}
    		...
    		try {
    			...
    			if ($res !== false) {
    				...
    				if ($re !== false) {
    					...
    					throw new \Exception(L('EDIT_ERROR'));
    				}
    			} else {
    				throw new \Exception(L('EDIT_ERROR'));
    			}

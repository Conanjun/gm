...
class MySQLReback {
    ...
    private function setFile() {
        ...
        $recognize = 'pmis_backup';
        $fileName1 =  $recognize . '_' . date('YmdHis') . '_' . mt_rand(100000000, 999999999) . '.sql';
        $fileName = $this->trimPath($this->config['path'] . self::DIR_SEP .$fileName1);
    ...
    public function backup() {
        ...
        if (!empty($this->content)) {
	        ...
	        if($fileName){
	        	$data  =  array(
	        		'name' => $fileName,
	        		'size' => getfilesize($fileName,$this->config['path']),
	        		'addtime' => time(),
	        		'uid' => UID,
	        		'uname' => get_nickname(UID),
		        );
	        	$id = M('backup')->add($data);
	        }
        }
        return $id;
    }

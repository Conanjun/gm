 ...
 *
 * @param        $str       需要转换的字符串
 * @param int    $length    需要从第几位开始隐藏
 * @param string $charset   编码格式
 *
 * @return bool
 */
function msubstr_length($str, $length = 5, $charset = "utf-8")
{
...
{
    $suffix = msubstr_length($str,$length);
 ...
 * @author huajie <banhuajie@163.com>
 */
function time_formatymd($time = NULL, $format = 'Y-m-d')
{
    if (empty($time)) {
        return '';
    }
 ...
 * @return string       用户名
 */
function get_username($uid = 0,$a = 0)
{
    static $list;
...
function get_email($uid = 0)
{
    ...
    /* 获取缓存数据 */
    if (empty($list)) {
        $list = S('sys_user_email_list');
    }
    /* 查找用户信息 */
    $key = "u{$uid}";
    $info = M('Ucenter_member')->field('email')->find($uid);
    if ($info !== false && $info['email']) {
        $nickname = $info['email'];
        $email = $list[$key] = $nickname;
        /* 缓存用户 */
        $count = count($list);
        $max = C('USER_MAX_CACHE');
        while ($count-- > $max) {
 ...
 * @return string       用户昵称
 */
function get_nickname($uid = 0,$a = 0)
{
    static $list;
    if (!($uid && is_numeric($uid))) { //获取当前登录用户名
	    if($a){
 ...
 * 记录行为日志，并执行该行为的规则
 */
function action_log_new($action = null)
{
    /*outtype,outkey,pid,action,uid,uname,addtime,comment,ext1,ext2,ext3,ext4,ext5*/
    //参数检查
    ...
    if (empty($action['uname'])) {
        $action['uname'] = get_nickname($action['uid']) ? get_nickname($action['uid']) : get_username($action['uid']);
    }
    if (empty($action['addtime'])) {
        $action['addtime'] = NOW_TIME;
    }

    $logid = M('Log')->add($action);

    if ($action['history']) {
        foreach ($action['history'] as $k => $v) {
            if (empty($v['logid'])) {
                $v['logid'] = $logid;
            }
            M('History')->add($v);
 ...
 * @param string $oss_path oss object对象（20180410/5acc5ced26f7c.jpg）前自动创建文件夹,后文件
 */
...
/**
 * 删除文件
 * @param string $object oss object对象（20180410/5acc5ced26f7c.jpg）
 */
 ...
 * @param string $object oss object对象（20180410/5acc5ced26f7c.jpg）
 */
function oss_exist_object($object){
	$oss=new_oss();
	$bucket=C('ALIOSS_CONFIG.BUCKET');
	$exist=false;
	if($object){
		try{
			$exist = $oss->doesObjectExist($bucket, $object);
		} catch(OssException $e) {
		}
	}
	return $exist;
...
/**
 * 获取oss文件地址
 * @param string $oss_path oss object对象（20180410/5acc5ced26f7c.jpg）
 */
function get_oss_url($oss_path){
	if (empty($oss_path)) {
		return '';
	}
	$alioss=C('ALIOSS_CONFIG');
	return 'http://'.$alioss['BUCKET'].'.'.$alioss['END_POINT'].'/'.$oss_path;
}

 ...
 *
 */
class OrderTaskSgController extends AdminController
{
	private $model;
    ...
    public function index(){
        ...
        $where = "1=1 and o.stuid = {$stuid} and t.tasktype = 'STUDIO_TASK_SG'";
    ...
    public function confirm(){
        ...
        if (IS_POST) {
            ...
            if($order['state'] < Order::ALREADY){
                $this->error(L('S_TASK_SG_ERROR_1'));
            }
            //是否是种植业务
            ...
            if(empty($_POST['task_image'])){
                $this->error(L('S_TASK_SG_MSG_2'));
            }
            $Model = M(); // 实例化一个空对象
            $Model->startTrans(); // 开启事务
			
            //订单图片
            $order_file = M('OrderFile')->where(array('order_sn'=>$order['order_sn']))->save(array('oid'=>$order_id));
            if(empty($order_file)){
            	//$this->json_error(L('S_TASK_SG_MSG_2'));
            }
            
            $o_exit = M('order_ext1')->where(array('oid'=>$order_id))->save(['pic3'=>$_POST['task_image']]);
            //上传数字化图片
//             $res_file = M('File')->add(['name'=>$_POST['task_images'], 'savepath'=>$_POST['task_image']]);
//             if($res_file){
//                 M('Order_file')->add(['oid'=>$order_id, 'file_id'=>$res_file,'ftype'=>'inner_model','addtime'=>time(),'uid'=>UID,'note'=>'']);
//             }
            ...
            $orders = M('auth_group') ->alias('g') ->join("ot_auth_group_access ag on g.id = ag.group_id and g.designer = 1 and g.stuid ={$stuid}")
                ->join("ot_order o on ag.uid = o.designer","left")
                ->field($field)->group('ag.uid')->order('count asc')->select();

            if(empty($orders[0]['uid'])){
                $this->error(L('S_TASK_SG_MSG_3'));
            }
            //给设计师生成设计方案任务
            $tname = sprintf(L('S_TASK_TASK_PROMPT_13'),$order['order_sn']);
            $tid = Order::add_task(array('oid' => $order_id, 'tname' =>$tname, 'tasktype' => 'STUDIO_TASK_SJS', 'isurgent'=>$order['isurgent'], 'uid' => $orders[0]['uid'] ,'note'=>$tname));


            //给设计师发消息
            $title = sprintf(L('S_TASK_TASK_PROMPT_15'),$order['order_sn']);
            $content = sprintf(L('S_TASK_TASK_PROMPT_16'),$order['stuname'],$order['order_sn']);
            $msg_des_id = Order::action_msg_log(array('fromid' => '', 'toid' => $orders[0]['uid'], 'addtime' => $time,'title' => $title,'content' => $content, 'outtype' => 'studio_dept' ,'outkey'=>$order_id));


            //给客户发消息
            $title = sprintf(L('S_TASK_TASK_PROMPT_1'),$order['order_sn']);
            $content = sprintf(L('S_TASK_TASK_PROMPT_5'),$order['stuname'],$order['order_sn'],L('DIGITIZED'));
            $msg_user_id = Order::action_msg_log(array('fromid' => '', 'toid' =>$order['uid'], 'addtime' => $time,'title' => $title,'content' => $content, 'outtype' => 'client_order' ,'outkey'=>$order_id));

            //写入订单操作记录
            $log_id = Order::action_order_log(array('oid' => $order_id, 'uid' =>$order['uid'], 'addtime' => $time, 'state' => Order::ASSIGNED ,'note'=>$finishnote));

            //更改订单状态
            $result = M('Order')->where("order_id=".$order_id)->save(array('state'=>Order::ASSIGNED,'designer'=> $orders[0]['uid'] ));

            //写入订单操作记录，修改订单状态 设计资料已接收
            //echo $msg_user_id.'--'.$msg_des_id.'--'.$tid.'--'.$log_id.'=='.$result.'===='.$res;
            //exit;
            if($msg_user_id&&$msg_des_id&&$tid&&$log_id&&$result&&$res){
                $Model->commit(); // 成功则提交事务
                $this->success(L('S_TASK_SG_SUC'), U('index'));
            }else{
                $Model->rollback(); // 否则将事务回滚
                $this->error(L('S_TASK_SG_ERROR'));
            }

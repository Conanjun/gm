	...
	$.extend(Modal.prototype, {
		show: function() {
			...
			if (!this.$modal.length) {
				this.$modal = $('<div class="modal fade" id="' + this.options.target.substr(1) + '">'+
			...
			if (this.options.width !== undefined) {
				this.$modal.find('.modal-dialog').width(this.options.width);
			}
			...
			if (this.options.height !== undefined) {
				this.$modal.find('.modal-dialog').height(this.options.height);
			}
			...
			if (this.options.remote !== undefined && this.options.remote != '' && this.options.remote !== '#') {
				...
				this.$modal.find('.inner').append('<div class="loader-inner pacman"><div></div><div></div><div></div><div></div><div></div></div>');
				this.$modal.find('.inner').load(this.options.remote, function() {
					if (spinner) {
						spinner.stop();
					}
					// if (self.options.cache) {
						// self.options.content.empty() = $(this).html();

...
class UploadHandler {
    ...
    public function getuploadType(){
        return $this->_return_mimetype($this->uploadName);
    }
    ...
    function get_suffix(){
        preg_match("|\.([a-z0-9]{2,4})$|i", $this->uploadName, $fileSuffix);
    ...
    public function handleUpload($uploadDirectory, $name = null){
        ...
        # non-chunked upload
            $path_name = $this->random_filename().'.'.$ext;
            $this->markName = $path_name;
            $target = join(DIRECTORY_SEPARATOR, array($uploadDirectory, $uuid, $path_name));
            if ($target){

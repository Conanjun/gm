...
class AopClient {
	...
	public function execute($request, $authToken = null, $appInfoAuthtoken=null) {
		...
		if (!$this->checkEmpty($this->alipayPublicKey)) {
			...
			// 获取结果sub_code
			$responseSubCode=$this->parserResponseSubCode($request,$resp,$respObject,'json');
	...
	// 查询对账单
    public function execute1($request, $authToken = null, $appInfoAuthtoken=null) {
        ...
        $sysParams['biz_content'] = $apiParams['biz_content'];
        //签名
        $sysParams["sign"] = $this->generateSign(array_merge($apiParams, $sysParams));

...
class UserController extends AdminController
{
    ...
    public function index()
    {
        ...
        $where = " ot_member.module = 'Studio' and ot_member.stuid ={$user['stuid']}";
        $filter= '';
    ...
    public function add($username = '', $password = '', $repassword = '', $email = '', $mobile = '')
    {
        if (IS_POST) {
            ...
            if (0 < $uid) { //注册成功
                $user = array('uid' => $uid, 'nickname' => $nickname, 'status' => 1, 'dept' => $deptid, 'sex' => $sex, 'birthday' => $birthday, 'stuid' => $user['stuid'],'module'=>'Studio');

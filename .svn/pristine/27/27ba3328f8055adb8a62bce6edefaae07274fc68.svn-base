$(function(){
	...
	$('span[ectype="inline_edit"]').click(function(){
		if(ajaxurl!=undefined&&ajaxurl!="") {
			...
			$('<input type="text">').css({border: '1px solid #ccc', width: '80%', height: '20px',textAlign:'left'})
				...
				.keyup(function (event) {
					if (event.keyCode == 13) {
						...
						if (sub) {
							$.get(ajaxurl, {id: s_id, column: s_name, value: $(this).attr('value')}, function (data) {
								if (data.done == false) {
									alert(data.msg);
...
//	给需要修改的图片添加异步修改行为
	$('img[ectype="inline_edit"]').click(function(){
		...
		$.get(url,{id:i_id,column:i_name,value:i_val},function(data){
			if(data === 'true')
			{
	...
	$('img[ectype="inline_edit"]').parent().attr('title','');
	//给列表有排序行为的列添加鼠标手型效果
	$('span[ectype="order_by"]').hover(function(){$(this).css({cursor:'pointer'});},function(){});
	//给列表的每一列添加排序行为
	$('span[ectype="order_by"]').live("click",function(){
		var s_name = $(this).attr('fieldname');
		var found   = false;
		...
		location.assign(window.location.origin+ '/index.php?' + params.join('&'));
	});
});
//检查提交内容的必须项

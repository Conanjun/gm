...
class AuthManagerController extends AdminController
{
    ...
    public function check_title()
    {
        ...
        $id = $_POST['id'];
        if ($id) {
            if (M('Auth_group')->where("title='{$title}' and id !=" . $id)->find())
                echo 'false';
            else
                echo 'true';
        } else {
            if (M('Auth_group')->where(['title' => $title])->find())
                echo 'false';
            else {
                echo 'true';
            }
    ...
    public function checkname(){
    	...
    	if(D("auth_group")->uniqueName($name,$id)){
    ...
    public function userlist(){
    	if(IS_POST){
    		...
    		$this->redirect('AuthManager/userlist?id=' . $id);
    		...
    		->join("ot_member m on m.uid=um.id and m.status=1 and m.module = 'Admin'")
    		->where($map_usr)->select();
    		
    		$ulist=M("UcenterMember")->alias("um")->join("inner join ot_member m on m.uid=um.id")->field("m.uid,m.nickname")->where(['m.module'=>'Admin'])->select();
